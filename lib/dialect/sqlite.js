...
Sqlite.prototype.visitBinary = function(binary) {
  ...
  if(binary.operator === '@@'){
    ...
    binary.operator = '@@';
    return ret;
  } else if (binary.operator.endsWith('LIKE') && this.config?.sqlite?.likeClauseEscapeCharacter) {
    options.suffix = `ESCAPE '${this.config.sqlite.likeClauseEscapeCharacter}'`;
  }
  return Sqlite.super_.prototype.visitBinary.call(this, binary, options);
};
